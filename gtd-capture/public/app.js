...
class GTDApp {
    ...
    firebaseAuth.onAuthStateChanged(async (user) => {
      if (user) {
        ...
        try {
          ...
          // Update user profile directory and check admin status
          await firestoreDb.ensureUserProfile();
          this.isAdmin = await firestoreDb.checkIsAdmin();
  ...
  async renderCurrentView() {
    ...
    switch (this.currentView) {
      ...
      // Admin views
      case 'adminUsers':
        this.activateView('adminUsersView');
        await this.renderAdminUsersView();
        break;
      case 'adminUserDetail':
        this.activateView('adminUserDetailView');
        await this.renderAdminUserDetailView();
        break;
    }
  }
...
Thanks!`,
  ...
  updateAdminNavVisibility() {
    const adminNav = document.getElementById('adminViewNav');
    const adminDivider = document.getElementById('adminNavDivider');
    if (adminNav) adminNav.style.display = this.isAdmin ? '' : 'none';
    if (adminDivider) adminDivider.style.display = this.isAdmin ? '' : 'none';
  }
  ...
  async renderAdminUsersView() {
    ...
    try {
      const users = await firestoreDb.getAllUserProfiles();

      if (users.length === 0) {
        container.innerHTML = `<div class="empty-state"><h3>No Users Found</h3><p>Users will appear here after they log in.</p></div>`;
        return;
      }
      ...
      for (const user of users) {
        const lastLogin = user.lastLoginAt?.toDate?.()
          ? this.getTimeAgo(user.lastLoginAt.toDate())
          : 'Never';
        const initial = (user.displayName || user.email || 'U').charAt(0).toUpperCase();

        html += `
          <div class="admin-user-card" onclick="app.showAdminUserDetail('${user.uid}', '${this.escapeHtml(user.displayName || user.email || '')}')">
            <div class="admin-user-avatar">${initial}</div>
            <div class="admin-user-info">
              <div class="admin-user-name">${this.escapeHtml(user.displayName || 'Unknown')}</div>
              <div class="admin-user-email">${this.escapeHtml(user.email || '')}</div>
            </div>
            <div class="admin-user-activity">
              <span class="admin-last-login">${lastLogin}</span>
            </div>
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="16" height="16" style="flex-shrink:0;color:var(--color-text-secondary)">
      ...
      container.innerHTML = '<div class="empty-state"><h3>Access Denied</h3><p>Admin permissions may not be configured yet.</p></div>';
    }
  }

  showAdminUserDetail(userId, displayName) {
    this.adminSelectedUserId = userId;
    this.adminSelectedUser = { displayName };
    this.navigateTo('adminUserDetail');
  }

  async renderAdminUserDetailView() {
    if (!this.isAdmin || !this.adminSelectedUserId) return;
    const container = document.getElementById('adminUserDetailContent');
    if (!container) return;

    const userId = this.adminSelectedUserId;
    const nameEl = document.getElementById('adminUserDetailName');
    if (nameEl) nameEl.textContent = this.adminSelectedUser?.displayName || 'User';

    container.innerHTML = '<div class="loading-state"><p>Loading user data...</p></div>';

    try {
      const [inbox, nextActions, waitingFor, projects, stats] = await Promise.all([
        firestoreDb.getOtherUserCollection(userId, 'inbox', 'timestamp', 'desc'),
        firestoreDb.getOtherUserCollection(userId, 'nextActions', '_created', 'desc'),
        firestoreDb.getOtherUserCollection(userId, 'waitingFor', '_created', 'desc'),
        firestoreDb.getOtherUserCollection(userId, 'projects', '_created', 'desc'),
        firestoreDb.getOtherUserStats(userId, 30)
      ]);

      const totalCaptures = stats.reduce((sum, s) => sum + (s.captured || 0), 0);
      const totalCompleted = stats.reduce((sum, s) => sum + (s.completed || 0), 0);
      const activeDays = stats.length;
      const activeActions = nextActions.filter(a => !a.completed);
      const activeProjects = projects.filter(p => p.status === 'active');
      ...
      html += `<div class="admin-stats-grid">
        <div class="admin-stat-card">
          <div class="admin-stat-value">${inbox.length}</div>
          <div class="admin-stat-label">Inbox</div>
        </div>
        <div class="admin-stat-card">
          <div class="admin-stat-value">${activeActions.length}</div>
          <div class="admin-stat-label">Actions</div>
        </div>
        <div class="admin-stat-card">
          <div class="admin-stat-value">${waitingFor.length}</div>
          <div class="admin-stat-label">Waiting</div>
        </div>
        <div class="admin-stat-card">
          <div class="admin-stat-value">${activeProjects.length}</div>
      ...
      html += `<div class="admin-section">
        <div class="admin-section-header">30-Day Activity</div>
        <div class="admin-activity-stats">
          <span>${activeDays} active days</span>
          <span>${totalCaptures} captured</span>
          <span>${totalCompleted} completed</span>
        </div>
      </div>`;

      // Inbox
      html += this.renderAdminItemSection('Inbox', inbox, item =>
        this.escapeHtml(item.content || item.text || ''),
        item => this.getTimeAgo(item.timestamp || item._created)
      );

      // Next Actions
      html += this.renderAdminItemSection('Next Actions', activeActions, item =>
        this.escapeHtml(item.action || item.content || ''),
        item => [
          item.contexts?.join(', '),
      ...
      // Waiting For
      html += this.renderAdminItemSection('Waiting For', waitingFor, item =>
        this.escapeHtml(item.action || item.content || ''),
        item => item.delegatedTo ? 'From: ' + this.escapeHtml(item.delegatedTo) : ''
      );

      // Projects
      html += this.renderAdminItemSection('Projects', projects, item =>
      ...
      container.innerHTML = '<div class="empty-state"><h3>Error</h3><p>Failed to load user data. Check admin permissions.</p></div>';
    }
  }

  renderAdminItemSection(title, items, contentFn, metaFn) {
    let html = `<div class="admin-section">
      <div class="admin-section-header">${title} <span class="admin-section-count">${items.length}</span></div>
      <div class="admin-item-list">`;

    if (items.length === 0) {
      html += '<div class="admin-item-empty">None</div>';
    } else {
      items.slice(0, 25).forEach(item => {
        html += `<div class="admin-item">
          <div class="admin-item-content">${contentFn(item)}</div>
          <div class="admin-item-meta">${metaFn(item)}</div>
        </div>`;
      });
      if (items.length > 25) {
        html += `<div class="admin-item-more">+ ${items.length - 25} more</div>`;
      }
    }
