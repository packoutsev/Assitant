...
// Map service names to credential file prefixes
const SERVICE_FILE_MAP = {
  gmail: "gmail_packouts",  // Fire leads are on matthew.roumain@1800packouts.com
  gchat: "gchat",
};

function loadAuth(service) {
  const filePrefix = SERVICE_FILE_MAP[service] || service;
  const credsPath = join(HOME, `.${filePrefix}_credentials.json`);
  const tokensPath = join(HOME, `.${filePrefix}_tokens.json`);
...
function parseFireleadsEmail(subject, body) {
  ...
  if (ownerBlock) {
    ...
    let cleaned = raw;
    // Remove phone-like patterns including (xxx) xxx-xxxx format and // separators
...
const NANO_OWNER_ID = "161300089";       // Anonno Islam's HubSpot owner ID
const NANO_CONTACT_ID = 359457110777;    // Anonno's HubSpot contact record

async function createHubSpotTask(lead) {
  if (!HUBSPOT_TOKEN) {
    log("  HubSpot: skipping (no HUBSPOT_TOKEN set)");
    return null;
  }

  const phones = [];
  if (lead.owner_phone) phones.push(lead.owner_phone);
  if (lead.renter_phone) phones.push(`Renter: ${lead.renter_phone}`);
  if (lead.commercial_phone) phones.push(`Business: ${lead.commercial_phone}`);

  const isAlarm = /alarm/i.test(lead.incident_type || "");
  const prefix = isAlarm ? "FIRE LEAD HOT" : "FIRE LEAD";

  const city = lead.city || "AZ";
  const subject = `${prefix}: ${lead.owner_name || lead.commercial_name || "Unknown"} - ${city} fire ${lead.date || ""}`.slice(0, 200);

  const bodyLines = [
    `FIRELEADS #${lead.incident_number || "?"}`,
    `${lead.address || "Unknown address"}`,
    lead.property_details || "",
    lead.property_value ? `Value: ${lead.property_value}` : "",
    lead.occupancy ? `Occupancy: ${lead.occupancy}` : "",
    "",
    "PHONES: " + (phones.length > 0 ? phones.join(" / ") : "NONE - door knock"),
    lead.commercial_name ? `Business: ${lead.commercial_name}` : "",
    "",
    lead.notes ? `FD Notes: ${lead.notes.slice(0, 300)}` : "",
    "",
    lead.services.length > 0 ? "Services: " + lead.services.join(", ") : "",
  ].filter(Boolean).join("\n");

  // Due tomorrow at 8am MST (15:00 UTC)
  const tomorrow = new Date();
  tomorrow.setUTCDate(tomorrow.getUTCDate() + 1);
  const dueDate = tomorrow.toISOString().slice(0, 10) + "T15:00:00.000Z";
  ...
  const taskData = {
    properties: {
      ...
      hubspot_owner_id: NANO_OWNER_ID,
      hs_task_status: "NOT_STARTED",
      hs_task_type: "CALL",
    },
    associations: [
      {
        to: { id: NANO_CONTACT_ID },
        types: [{ associationCategory: "HUBSPOT_DEFINED", associationTypeId: 204 }],
      },
    ],
  };

  const resp = await fetch(`${HUBSPOT_API}/crm/v3/objects/tasks`, {
    method: "POST",
    headers: {
  ...
  if (!resp.ok) {
    ...
    throw new Error(`HubSpot task creation failed (${resp.status}): ${errBody}`);
  }
...
async function main() {
  ...
  for (const msg of messages) {
    ...
    if (isDryRun) {
      ...
      // Create HubSpot task for Nano
      try {
        const hsResult = await createHubSpotTask(lead);
        if (hsResult) log(`  HubSpot: task ${hsResult.id} created`);
      } catch (e) {
        log(`  HubSpot task failed (non-blocking): ${e.message}`);
      }
