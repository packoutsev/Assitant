 ...
 *
 * OAuth2 tokens persisted in GCS. Automatic refresh on expiry.
 *
 * Env: GCHAT_CLIENT_ID, GCHAT_CLIENT_SECRET, GCS_BUCKET, GCHAT_SPACE_NAME
 */
...
export async function postMultipleToChat(messages) {
  const results = [];
  for (const msg of messages) {
    const result = await postToChat(msg);
    results.push(result);
