...
export default function CallScreen({ onCallEnd }: Props) {
  ...
  const handleEndCall = useCallback(() => {
    ...
    navigate('/call-trainer/review');
  }, [elapsed, type, difficulty, scenario.characterName, onCallEnd, navigate]);
    ...
    <div className="min-h-screen bg-navy flex flex-col">
      ...
      <div className="flex-1 flex flex-col items-center justify-center gap-6 px-6">
        <div className="relative">
          ...
          >
            {scenario.characterName.split(' ').map((n) => n[0]).join('')}
          </div>
          {status === 'active' && (
            <>
              <div className="absolute inset-0 rounded-full bg-emerald-500/40 animate-pulse" style={{ animation: 'pulse-ring 1.5s ease-out infinite' }} />
              <div className="absolute inset-0 rounded-full bg-emerald-500/20 animate-pulse" style={{ animation: 'pulse-ring 1.5s ease-out infinite 0.5s' }} />
            </>
          )}
          ...
          <div className="flex items-center gap-1 h-10">
            {Array.from({ length: 12 }).map((_, i) => (
              <div
                ...
                style={{
                  animation: `waveform 0.8s ease-in-out infinite ${i * 0.1}s`,
                  height: '8px',
                }}
              />
            ))}
      ...
      {/* Transcript panel */}
      <div className="mx-6 mb-4 bg-black/30 rounded-xl border border-white/10 max-h-48 overflow-y-auto">
        {transcript.length === 0 ? (
